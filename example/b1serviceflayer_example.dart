...
main() async {
  const url = 'http://hanasrv:50001/b1s/v2/';
  const user = 'manager';
  const pwd = '@demopwd';
  const companyDB = 'SBODEMO';
  ...
  try {
    ...
    var activitiesJson = await b1s.queryAsync('Activities');
    Map<String, dynamic> activitiesMap = json.decode(activitiesJson!);
    List<dynamic> activityList = activitiesMap['value'];
    print(
        '${activityList.length} activities returned in ${b1s.exetutionMilliseconds} ms');
    activityList.forEach((activity) {
      print(
          'Activity ${activity["ActivityCode"]} Completed ${activity["Status"]} Notes ${activity["Notes"]} ');
    });
    ...
    await b1s.updateAsync(
        entityName: 'Activities($activityCodeToComplete)',
        entityJSON: json.encode(activityList[0]));

    print('Fetching updated activity ...');
    var activityJson =
        await b1s.queryAsync('Activities($activityCodeToComplete)');
    Map<String, dynamic> activityMap = json.decode(activityJson!);
    print(
        'Activity ${activityMap["ActivityCode"]} Completed ${activityMap["Status"]} Notes ${activityMap["Notes"]} in ${b1s.exetutionMilliseconds} ms');

    print('Adding a new activity ...');
    activityJson = await addActivity(b1s, 12);
    activityMap = json.decode(activityJson);
    print(
        'Activity created with code ${activityMap["ActivityCode"]} Notes ${activityMap["Notes"]} in ${b1s.exetutionMilliseconds} ms');

    print('Deleting the new activity ...');
    activityCodeToComplete = activityMap['ActivityCode'];
    await b1s.deleteAsync(
        entityName: 'Activities($activityCodeToComplete)',
    ...
    print(
        'Number of activities ${activityList.length} returned in ${b1s.exetutionMilliseconds} ms');
    activityList.forEach((activity) {
      print(
          'Activity ${activity["ActivityCode"]} Completed ${activity["Status"]} Notes ${activity["Notes"]} ');
    });
...
Future<String> addActivity(B1ServiceLayer b1s, int userId) async {
  ...
  activityData['StartDate'] =
      '${DateTime(2019, 9, 7)}'; //Automatically convert Dates and Times to strings in POST/PUT/PATCH
  ...
  return await b1s.createAsync(
      entityName: 'Activities', entityJSON: json.encode(activityData));
}
